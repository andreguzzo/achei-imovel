...
import {
  Loader2, Mail, UserPlus, BarChart3, TrendingUp, Plus, Users, Calendar, DollarSign, FileText, Handshake, CalendarDays,
} from "lucide-react";
...
const DashboardSalesTab = ({ userId }: DashboardSalesTabProps) => {
  ...
  const fetchData = useCallback(async () => {
    ...
    setPipelineItems((pipelineRes.data as PipelineItem[]) ?? []);
    ...
    if (parts.length > 0) {
      ...
      parts.forEach((p) => {
        ...
        p.partner_name = nameMap[partnerId] ?? "Corretor";
      });
    }
  ...
  // Pipeline KPIs
  const totalLeads = pipelineItems.filter((i) => i.stage === "lead").length;
  const visitsScheduled = pipelineItems.filter((i) => i.stage === "visit_scheduled").length;
  const activeProposals = pipelineItems.filter((i) => ["proposal", "negotiation"].includes(i.stage)).length;
  const closedWon = pipelineItems.filter((i) => i.stage === "closed_won").length;
  const totalCommission = pipelineItems.filter((i) => i.stage === "closed_won").reduce((acc, i) => acc + (i.commission_value ?? 0), 0);

  const funnelData = STAGES.filter((s) => s.key !== "closed_lost").map((s) => ({
    name: s.label,
    value: pipelineItems.filter((i) => i.stage === s.key).length,
  }));

  const byStage = STAGES.reduce<Record<string, PipelineItem[]>>((acc, s) => {
    acc[s.key] = pipelineItems.filter((i) => i.stage === s.key);
    return acc;
  }, {});
